...
public class MonteCarloDriver {
	private final int xdim = 100; //total cell dimensions in x-direction
	private final int ydim = 100; //total cell dimensions in y-direction
	private final int zdim = 60; //total cell dimensions in z-direction (thickness)
	...
	private CalcEfficiency Efficiency; //efficiency, rho, used to determine when simulation reaches steady state
	private double excirate; //exciton generation rate
...
//		}
	...
	private void processEvent(Event temp, PrintWriter out) throws IOException {
		...
		if(particles[x][y][z] == null & !action.equals("excigen")) {
			...
			printStuff(out);
			int[] loc = ExciGenerator.genExciton(xdim, ydim);
			Event exci = new Event(excirate*-Math.log(RandomGenerator.randDouble()), "excigen", loc[0], loc[1], loc[2], 0,0,0);
			eventQueue.add(exci);
		}
