...
public abstract class Particle {
	...
	protected static void genDistances() {
		...
		double sel1 = 69; //percentage of 100 IP sites
		double sel2 = 91; //percentage of 100 OOP sites
			...
			for (double dy = -2.0; dy < 3; dy++) {
				for (double dz = -2.0; dz < 3; dz++) {
					...
					if (latticeselector <= sel1) 	{
						dist = Math.pow(a*2*dx, 2.0) + Math.pow(a*2*dy, 2.0);	
						...
						dist = Math.pow(b*2*dx, 2.0) + Math.pow(b*2*dy, 2.0);
						if (latticeselector2 <= sel2) {
							distances[index] = Math.sqrt(dist + Math.pow(a*dz, 2.0));
						}
						else {
		...
		for (double dx = -2.0; dx < 3; dx++) {
			for (double dy = -2.0; dy < 3; dy++) {
				for (double dz = -2.0; dz < 3; dz++) {
					...
					if (latticeselector <= sel1)	{
						dist = Math.pow(a*2*(dx-location[0]), 2.0) + Math.pow(a*2*(dy-location[1]), 2.0);
						...
						dist = Math.pow(b*2*(dx-location[0]), 2.0) + Math.pow(b*2*(dy-location[1]), 2.0);
